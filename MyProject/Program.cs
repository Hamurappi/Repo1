...
namespace MyProject
{
    ...
    class StackDemo
    {
        static void Main()
        {
            ...
            Console.WriteLine("Поместить символы A-J в стек 1.");
            for (i = 0; !(stk1.IsFull()); i++)
                stk1.Push((char)('A' + i));
            if (stk1.IsFull()) Console.WriteLine("Стек 1 заполнен.");
            // Вывести содержимое стека 1.
            Console.Write("Содержимое стека 1: ");
            while ( !(stk1.IsEmpty()) )
            {
                ch = stk1.Pop();
                Console.Write(ch);
            ...
            if (stk1.IsEmpty() ) Console.WriteLine("Стек 1 пуст.");
            // Поместить дополнительные символы в стек1.
            Console.WriteLine("Вновь поместить символы A-J в стек stk1.");
            for (i = 0; !(stk1.IsFull()); i++)
                stk1.Push((char)('A' + i));
            // А теперь извлечь элементы из стека 1 и поместить в стек 2.
            // В итоге элементы в стеке 2 сохраняются в обратном порядке.
            Console.WriteLine("А теперь извлечь символы из стека stk1\n"+"и поместить в стек stk2.");
            while ( !(stk1.IsEmpty()) )
            {
                ch = stk1.Pop();
                stk2.Push(ch);
            }
            Console.Write("Содержимое стека 2: ");
            while ( !(stk2.IsEmpty()) )
            {
                ch = stk2.Pop();
                Console.Write(ch);
            ...
            Console.WriteLine("Поместить 5 символов в стек 3: ");
            for(i=0;i<5;i++)
                stk3.Push((char)('A'+i));
